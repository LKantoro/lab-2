...
graph1_tibble2 %>% ggplot(aes(x = value, fill = map_region)) +
  geom_dotplot(binwidth = .02) + facet_grid(rows = vars(map_region)) +
  theme(panel.grid.minor = element_blank(), panel.grid.major.y = element_blank(), 
  plot.title.position = "plot", axis.text.y = element_blank()) + 
  #theme_minimal() +
  labs(x = "Proportion", 
       y = "",
       title = "Proportion of People Who Believe Vaccines Are Very Effective By Country and Region")
...
graph2_df = left_join(new_graph1_tibble, world_sf, by = join_by(country == NAME))

bins = c(0, 10, 20, 50, 100, 200, 500, 1000, Inf)

pal = colorBin("YlOrRd", domain = apple$value, bins = bins)

class(apple)

graph2_df = as.data.frame(graph2_df)

leaflet(graph2_df) %>% addTiles() %>% addPolygons(fillColor = ~value, weight = 2)
...
percent_effective_and_safe = left_join(percent_effective_and_safe, new_graph1_tibble2[c(1,7)], by = join_by(country == country))
...
fig = plot_ly(data = percent_effective_and_safe, x = ~perc_safe, y = ~perc_effective,
    type = "scatter", text = ~country, color = ~map_region) %>%
  layout(plot_bgcolor='white',
    xaxis = list(ticksuffix = "%", range = list(0, 100), title = ""),
    yaxis = list(ticksuffix="%", range = list(0, 100), title = ""),
    title = "Belief in Vaccine Safety Versus Belief in Vaccine Effectiveness in Different Countries")
