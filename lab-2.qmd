...
pal = colorBin("RdYlGn", domain = graph2_df$value, bins = bins)
...
leaflet(graph2_df) %>% addTiles() %>% 
  addPolygons(fillColor = ~pal(value), weight = .2,  opacity = 1, color = 'black', popup = ~country_name, label = ~value) %>%
  addLegend(pal = pal, values = ~value, position = "bottomleft", title = "",
            na.label = "Data Not Available", labFormat = labelFormat(suffix = "%")) %>%
  addControl(c("People Who Believe Vaccines are Safe"), position = "topleft")
  #
