...
regionsstring = regionsstring %>% str_replace_all('=', ',')

regionsstring = str_split(regionsstring, ',')

regions_df = data.frame(matrix(unlist(regionsstring), ncol = length(regionsstring) * 2, byrow = TRUE))

regions_df = regions_df[1:19, 1:2]
...
regions_df = regions_df %>% rename('region_code' = X1)

regions_df = regions_df %>% rename('region_name' = X2)


regions_df$region_code = as.numeric(regions_df$region_code)


regions_df['map_region'] = case_when(regions_df[['region_code']] %in% c(9,10,11,12, 18) ~ 'Asia and Australia',
          regions_df[['region_code']] %in% c(1,2,4,5) ~ 'Sub-Saharan Africa',
          regions_df[['region_code']] %in% c(6,7,8) ~ 'Americas',
          regions_df[['region_code']] %in% c(15,16,17) ~ 'Europe',
          regions_df[['region_code']] %in% c(3,13) ~ 'Middle East and North Africa',
          regions_df[['region_code']] == 14 ~ 'Former Soviet Union',
          regions_df[['region_code']] == 0 ~ 'Not Assigned')

regions_df

wgm2018_full_new2 = left_join(wgm2018_full_new, regions_df, by = join_by(Regions_Report == region_code))


#0=Not assigned, 
#1=Eastern Africa,2=Central Africa,3=North Africa,4=Southern Africa,5=Western Africa,
#6=Central America and Mexico,7=Northern America,8=South America,
#9=Central Asia,10=East Asia,11=Southeast Asia,12=South Asia,
#13=Middle East,
#14=Eastern Europe,15=Northern Europe,16=Southern Europe,17=Western Europe,
#18=Aus/NZ,
...
country_counts = wgm2018_full_new2 %>% group_by(country) %>% summarise(total_count = n())



vaccine_safe_counts = wgm2018_full_new2 %>% group_by(country, Q25) %>% summarise(total_count = n())

vaccine_safe_counts = filter(vaccine_safe_counts, Q25 == '1')


new_graph1_tibble = left_join(country_counts, vaccine_safe_counts, by = join_by(country == country))



new_graph1_tibble$value = new_graph1_tibble$total_count.y/new_graph1_tibble$total_count.x

wgm2018_full_new2[61:63]


new_graph1_tibble2 = left_join(new_graph1_tibble, regions_df, by = join_by(Regions_Report == region_code))


new_graph1_tibble2 %>% ggplot(aes(x = value)) +
  geom_dotplot()
...
vaccine_effective_counts = wgm2018_full_new %>% group_by(country, Q26) %>% summarise(total_count = n())
...
percent_effective = left_join(country_counts, vaccine_effective_counts, by = join_by(country == country))


percent_effective_and_safe = left_join(percent_effective, vaccine_safe_counts, by = join_by(country == country))

percent_effective_and_safe


percent_effective_and_safe = percent_effective_and_safe %>% rename('num_safe'= total_count)

percent_effective_and_safe = percent_effective_and_safe %>% rename('num_effective'= total_count.y)

percent_effective_and_safe = percent_effective_and_safe %>% rename('total'= total_count.x)

percent_effective_and_safe

percent_effective_and_safe$perc_safe = percent_effective_and_safe$num_safe/percent_effective_and_safe$total * 100

percent_effective_and_safe$perc_effective = percent_effective_and_safe$num_effective/percent_effective_and_safe$total * 100
...
fig <- plot_ly(data = percent_effective_and_safe, x = ~perc_safe, y = ~perc_effective, type = "scatter", text = ~country) %>% layout(
    plot_bgcolor='white',
    xaxis = list(ticksuffix = "%", range = list(0, 100)),
    yaxis = list(ticksuffix="%", range = list(0, 100)),
    title = "Belief in Vaccine Safety Versus Effectiveness in Different Countries") %>% 
  add_lines(x = ~perc_safe, y = fitted(best_fit_line))
